...
job_logs_queue = deque(maxlen=100)  # Avoid memory bloat
config_store = {}  # Global configs for all scripts
hot_assets = {}  # Shared objects/modules/resources

def execute_script(script_name, script_args, job_id):
    """Runs a script as a separate process and logs output."""
    try:
        # Append config path to script args if not using --noconfig
        if "--noconfig" not in script_args:
            script_args.append(f"--config={json.dumps(config_store)}")
...
def handle_client(conn):
    ...
    try:
        ...
        if command == "run":
            ...
            conn.sendall(f"Config '{key}' set to '{value}'.\n".encode("utf-8"))

        elif command == "config-get":
            key = parts[1]
            value = config_store.get(key, "Not set")
            conn.sendall(f"Config '{key}': {value}\n".encode("utf-8"))

        elif command == "asset-register":
            key = parts[1]
            value = " ".join(parts[2:])
            hot_assets[key] = value
            conn.sendall(f"Asset '{key}' registered.\n".encode("utf-8"))

        elif command == "asset-get":
            key = parts[1]
            value = hot_assets.get(key, "Not found")
            conn.sendall(f"Asset '{key}': {value}\n".encode("utf-8"))
